...
## 2025-10-31 Progress
- Added a `SwiftPieTestSupport` target powered by SwiftNIO with `withTestServer` helpers (sync + async) that spin up an on-process server and expose request recordings.
...
- Added `Tests/SwiftPieTests/TestHTTPServerTests.swift` exercising happy-path GET/POST flows, status codes, cookie issuance, and redirect handling via `URLSession`.
