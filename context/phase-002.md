...
## 2025-10-30 Progress
- Added `SwiftPieTests/RequestParserTests.swift` scenarios mirroring `TestItemParsing`, `TestQuerystring`, `TestLocalhostShorthand`, and method inference cases via the Swift Testing `#expect` macro.
...
## 2025-10-31 Follow-up
- Tightened CLI option parsing so unsupported flags such as `--unknown` trigger `EX_USAGE` with a dedicated diagnostic before request parsing, covered by a new `CLIRunnerTests` case.
